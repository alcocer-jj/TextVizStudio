...
for i in range(num_models):
    with st.expander(f"Configure Model {i+1}", expanded=(i==0)):
        ...
        se_opts = sorted(set.intersection(*(SUPPORTED_SE[e] for e in ests)))
        se_type = st.selectbox("Standard errors", se_opts, key=f"se_{i}")
        cl = None
        if se_type == "Clustered":
            cl = st.selectbox("Cluster variable", data.columns, key=f"cl_{i}")
        configs.append({
            "dv": dv, "ivs": ivs, "ests": ests,
            "ent": ent, "time": time,
            "mg": mg, "ms": ms,
            "se": se_type, "cl": cl
        })
...
if st.button("Run Models"):
    ...
    for idx, cfg in enumerate(configs, start=1):
        ...
        stats_cov = COMMON_STATS_SE[cfg['se']]
        panel_cov = PANEL_SE[cfg['se']]
        mixed_cov = MIXED_SE[cfg['se']]
        for est in cfg['ests']:
            key = f"Model{idx}_{est.replace(' ','')}"
            try:
                if est == "Zero-Inflated NB":
                    # ZINB: scaling option
                    scale = st.checkbox(f"Scale IVs for ZINB Model {idx}", key=f"zinb_scale_{idx}")
                    optimizer = st.selectbox(
                        f"Optimizer for ZINB Model {idx}", ["BFGS", "Newton"], key=f"zinb_opt_{idx}"),
                    method = 'bfgs' if optimizer == "BFGS" else 'newton'
                    # prepare data copy
                    df_zinb = data.copy()
                    if scale:
                        for v in cfg['ivs']:
                            df_zinb[v] = (df_zinb[v] - df_zinb[v].mean()) / df_zinb[v].std()
                    # inflation intercept-only toggle
                    use_intercept = st.checkbox(
                        f"Intercept-only inflation for ZINB Model {idx}", key=f"zinb_int_{idx}"
                    )
                    if use_intercept:
                        ex_infl = pd.DataFrame({'const': 1}, index=df_zinb.index)
                    else:
                        ex_infl = df_zinb[cfg['ivs']]
                    mod = count.ZeroInflatedNegativeBinomialP.from_formula(
                        form,
                        exog_infl=ex_infl,
                        data=df_zinb,
                        inflation="logit"
                    )
                    res = mod.fit(method=method, maxiter=100, disp=False)
                    if not res.mle_retvals.get('converged', False):
                        st.warning(f"ZINB Model {idx} did not converge; try centering or simpler inflation.")
                elif ESTIMATOR_MAP[est]["func"]:
                    mod = ESTIMATOR_MAP[est]["func"](form, data)
                    covargs = dict(stats_cov)
